...
const versions = async project => {
  const html = await fetch_text(incubator_releases_url(project))
...
const files = async (project, version) => {
  const html = await fetch_text(release_files_url(project, version))
  const $ = cheerio.load(html)
...
const fetch_url = async url => {
  ...
  if (resp.status !== 200) {
    throw new Error(`release file at ${url} unavailable, status code: ${resp.status}`)
  }

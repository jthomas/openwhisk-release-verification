...
const paths = (archive_stream, matching) => {
  return new Promise((resolve, reject) => {
    const matching_paths = []
    const tar_stream = archive_stream.pipe(tar.t())

    tar_stream.on('entry', entry => {
      if (matching(entry.path)) {
        matching_paths.push(entry.path)
      }
    })
    tar_stream.on('end', () => resolve(matching_paths))
    tar_stream.on('error', reject)
  })
}

// list of excluded file paths sourced from:
// https://github.com/apache/incubator-openwhisk-release/blob/449e28cca638dc7be8fda4b538b12c892ccbc10d/tools/rcverify.sh#L148-L158
const excluded = path => {
  const EXCLUDED_PATHS = /\.py$|\.tar$|\.tgz$|.sh$|\.php$|gradlew$|gradlew.bat$|\.zip$|\.gz$|\.jar$|node_modules\/$|\.gradle\/$/
  return !!path.match(EXCLUDED_PATHS)
}

module.exports = { files, paths, excluded }

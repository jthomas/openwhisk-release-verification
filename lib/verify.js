...
const disclaimer = (err, file, contents, name) => {
  const missing_disclaimer = !!(err && err.code == 'ENOENT')
  logger.log(`file (${file}) is not included in archive: ${missing_disclaimer}`)
...
const readme = (err, file, contents) => {
  ...
  logger.log(`file (${file}) does not have incubation disclaimer: ${missing_disclaimer}`)

  return missing_disclaimer
}
  
const license = (err, file, contents) => check_file_contents(

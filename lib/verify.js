...
const disclaimer = (contents, name) => matches_template('DISCLAIMER.txt', `Apache OpenWhisk ${name}`, contents)
const license = contents => matches_template('LICENSE.txt', '', contents)
const notice = (contents, name) => matches_template('NOTICE.txt', `Apache OpenWhisk ${name}`, contents)
...
const archive_files = async (archive_stream, project_id) => {
  const files_valid = {
  ...
  const results = file_names.reduce((result, name) => {
    result[name] = contents[name] && files_valid[name](contents[name], project_id)
    return result
  }, {})

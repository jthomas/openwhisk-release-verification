...
const hash = async (file_stream, hash_file, name) => {
  return new Promise((resolve, reject) => {
    ...
    hmac.on('readable', () => {
      ...
      console.log(`file (${name}) calculated hash: ${stream_hash}`)
      console.log(`file (${name}) hash from file:  ${sha512.signature}`)
      resolve({valid})
    })
...
const signature = async (file_stream, signature, public_keys, name) => {
  ...
  options.publicKeys.forEach(pk => console.log(`file (${name}) signature checked against public key with id:`, pk.getKeyId().toHex()))
  if (valid) {
    console.log(`file (${name}) signed by key with id:`, verified.signatures[0].keyid.toHex())
  } else {
    console.log(`file (${name}) signature validation failed, not signed using available public keys`)
  }
...
const matches_template = (file, id, prefix, contents) => {
  ...
  if (prefix) {
    console.log(`file (${file}) checking ${id} has correct project name:`, correct_project_name)
  }

  console.log(`file (${file}) checking ${id} has valid contents:`, correct_contents)

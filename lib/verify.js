...
const disclaimer = (err, file, contents, name) => {
  ...
  logger.log(`file (${file}) ${name} is not included in archive: ${missing_disclaimer}`)

  return missing_disclaimer
}

// README should not include ASF incubation disclaimer notice.
const readme = (err, file, contents, name) => {
  const missing_disclaimer = !contents.match(/undergoing incubation at The Apache Software Foundation/)
  logger.log(`file (${file}) ${name} does not have incubation disclaimer: ${missing_disclaimer}`)
